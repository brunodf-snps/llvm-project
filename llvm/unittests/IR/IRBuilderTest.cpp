...
TEST_F(IRBuilderTest, NoAlias) {
  ...
  AllocaInst *VarP = Builder.CreateAlloca(Builder.getPtrTy());
  ...
  Value *Pointer = Builder.CreateLoad(Builder.getPtrTy(), VarP);
  CallInst *NAP =
  ...
  EXPECT_TRUE(NAD->onlyAccessesInaccessibleMemory());
...
TEST_F(IRBuilderTest, ProvenanceNoAliasAndPtrProvenance) {
  IRBuilder<> Builder(BB);
  AllocaInst *Var1 = Builder.CreateAlloca(Builder.getInt8Ty());
  ...
  CallInst *NAG = cast<CallInst>(Builder.CreatePtrProvenance(Pointer, SNAP));
...
TEST_F(IRBuilderTest, NoAliasCopyGuard) {
  ...
  IRBuilder<> Builder(BB);
  StructType *ST =
      StructType::create(BB->getContext(), {Builder.getPtrTy()});

  // pointer to incoming struct
  AllocaInst *VarIn = Builder.CreateAlloca(Builder.getPtrTy());
  ...
  auto *SrcPointer = Builder.CreateLoad(Builder.getPtrTy(), VarIn);

  auto PtrNull = ConstantPointerNull::get(Builder.getPtrTy());
  auto *PassThrough = Builder.CreateNoAliasCopyGuard(
      SrcPointer,
      PtrNull, // out-of-function __restrict declaration
      nullptr, // no restrict offsets
      AScope);
  EXPECT_EQ(PassThrough, SrcPointer);

  MDBuilder::NoAliasOffsetsField F(0, 8, 1);
  MDNode *NAO = MDB.createNoAliasOffsets(8, {F});
  ...
  EXPECT_EQ(cast<MetadataAsValue>(
                GP->getArgOperand(Intrinsic::NoAliasCopyGuardIndicesArg))
                ->getMetadata(),
            NAO);
  EXPECT_EQ(cast<MetadataAsValue>(
