...
#undef INSTRUCTION
  ...
  case Intrinsic::noalias: {
    ...
    if (!isa<ConstantPointerNull>(DA) && !isa<UndefValue>(DA)) {
      // FIXME: this should look through phi nodes like provenance_noalias?
      auto *DeclSite = dyn_cast<CallBase>(DA);
      Check(DeclSite, "llvm.noalias arg1 must refer to a llvm.noalias.decl",
            Call);
      Check(DeclSite->getIntrinsicID() == Intrinsic::noalias_decl,
            "llvm.noalias arg1 must refer to a llvm.noalias.decl", Call);
      Check((DeclSite->getArgOperand(Intrinsic::NoAliasDeclObjIdArg) ==
             Call.getArgOperand(Intrinsic::NoAliasIdentifyPObjIdArg)),
            "llvm.noalias objId arg must match with llvm.noalias.decl", Call);
      Check((DeclSite->getArgOperand(Intrinsic::NoAliasDeclScopeArg) ==
    ...
    if (!isa<ConstantPointerNull>(DA) && !isa<UndefValue>(DA)) {
      ...
      for (auto *CB : NoAliasDeclarations) {
        Check(CB,
              ...
              Call);
        Check(CB->getIntrinsicID() == Intrinsic::noalias_decl,
              "llvm.provenance.noalias arg1 must refer to a llvm.noalias.decl",
              Call);
        Check(
            ...
            "llvm.noalias.decl",
            Call);
        Check((CB->getArgOperand(Intrinsic::NoAliasDeclScopeArg) ==
