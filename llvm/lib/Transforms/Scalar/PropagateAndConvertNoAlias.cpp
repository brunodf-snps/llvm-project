...
// llvm.experimental.ptr.provenance intrinsics.
//
...
// -- llvm.experimental.ptr.provenance a, prov_a -> prov_a
//
...
//
// The algorithm:
// - We start from the llvm.noalias and llvm.experimental.ptr.provenance
// instructions
...
static void propagateInstructionsForProvenance(
  ...
  while (!InstructionsForProvenance.empty()) {
    ...
    if (auto *CB = dyn_cast<CallBase>(I4SC)) {
      if (CB->getIntrinsicID() == Intrinsic::experimental_ptr_provenance) {
        // llvm.experimental.ptr.provenance: delegate to ptr_provenance (operand
        // 1)
    ...
    while (!WorkList.empty()) {
      ...
      for (auto &UOp : I->uses()) {
        ...
        if (isa<SelectInst>(U)) {
          ...
          // ======================================== load -> { ptr }
          if (UOp.getOperandNo() == LoadInst::getPtrProvenanceOperandIndex())
          ...
          // ======================================== store -> { val, ptr }
          if (UOp.getOperandNo() == StoreInst::getPtrProvenanceOperandIndex())
            continue; // tracking on provenance -> ignore
          ...
          // ======================================== insertvalue -> { val }
          // track for injecting llvm.experimental.ptr.provenance
          ...
          case Instruction::AtomicCmpXchg:
            // NOTE: every instruction handled here should also be handled in
            // Phase 3 of ::doit (normally the last 'else' case)
            OnlyGuard = true;
            break;
          default:
#ifndef NDEBUG
            llvm::dbgs() << "ERROR: unhandled case:";
            U->dump();
#endif
...
static Value *createBitOrPointerOrAddrSpaceCast(Value *V, Type *T,
  ...
  if (Entry == nullptr) {
    ...
    if (isa<PHINode>(I))
      IP = BB->getFirstNonPHIIt();
    else
      ++IP;

    IRBuilder<> Builder(BB, IP);
    Entry = Builder.CreatePointerBitCastOrAddrSpaceCast(V, T);
  }
  return Entry;
}
          ...
          if ((DepI->getNumIncomingValues() == 2) &&
            ...
            if (SelfDep0 || SelfDep1) {
              ...
              if (isValidProvenanceNoAliasInsertionPlace(
                ...
                NewSNA->setAAMetadata(I->getAAMetadata());
    ...
    while (!worklist.empty()) {
      ...
      for (auto *SNIUser_ : worker->users()) {
        ...
        if (isa<PHINode>(SNIUser)) {
          ...
          // load/store/provenance.experimental.ptr.provenance -> stop looking
          if (auto *CB = dyn_cast<CallBase>(SNIUser)) {
            auto CBIID = CB->getIntrinsicID();
            if (CBIID == Intrinsic::experimental_ptr_provenance) {
              assert(CB->getOperand(1) == worker &&
                     "a experimental.ptr.provenance provenance should be "
                     "linked to "
                     "operand 1");
...
static void RetrieveDeadBasicBlocks(Function &F,
  ...
  LLVM_DEBUG(llvm::dbgs() << "- There are " << out_DeadBasicBlocks.size()
                          << " unreachable BB on a total of " << F.size()
                          << "\n");
}
...
bool PropagateAndConvertNoAliasPass::doit(Function &F, DominatorTree &DT) {
  ...
  for (auto &BB : F) {
    ...
    for (auto &I : BB) {
      if (auto CB = dyn_cast<CallBase>(&I)) {
        ...
        } else if (ID == Intrinsic::experimental_ptr_provenance) {
          LLVM_DEBUG(llvm::dbgs() << "-- found intrinsic:" << I << "\n");
          auto Op0 = I.getOperand(0);
  ...
  LLVM_DEBUG(
      llvm::dbgs()
      << "- Looking through degenerated llvm.experimental.ptr.provenance\n");
  ...
  // Helper lambda for inserting a new experimental.ptr.provenance
  auto setNewNoaliasArgGuard = [&](Instruction *I, unsigned Index,
    ...
    if (ArgGuard == nullptr) {
      ...
      ArgGuard = BuilderForArgs.CreatePtrProvenance(
  ...
  for (auto CloneableInst : CreationList) {
    ...
    while (!Worklist.empty()) {
      ...
      if (HandledIt == Handled.end()) {
        // This can happen after propagation of a
        // llvm.experimental.ptr.provenance
        Worklist.pop_back();
        I2NewV[I] = I;
        ...
        if (isa<SelectInst>(I)) {
          ...
          if (Deps[0]) {
            if (!LI->hasPtrProvenanceOperand() ||
                ...
                (LI->getPointerOperand() == LI->getPtrProvenanceOperand())) {
              LI->setPtrProvenanceOperand(createBitOrPointerOrAddrSpaceCast(
          ...
          if (Deps[1]) {
            if (!SI->hasPtrProvenanceOperand() ||
                ...
                (SI->getPointerOperand() == SI->getPtrProvenanceOperand())) {
              SI->setPtrProvenanceOperand(createBitOrPointerOrAddrSpaceCast(
          ...
          // Track generated experimental_ptr_provenance also in I2NewI
          assert(Deps.size() == 1 &&
          ...
          if (CB) {
            ...
            case Intrinsic::noalias: {
              ...
              // P)
              NewI->setAAMetadata(I->getAAMetadata());
              continue;
            }
            case Intrinsic::experimental_ptr_provenance: {
            ...
            assert(isa<ReturnInst>(I) || isa<AtomicRMWInst>(I) ||
                   isa<AtomicCmpXchgInst>(I));
          }

          // Introduce a experimental_ptr_provenance for every argument that is
          // annotated
  ...
  for (Instruction *I : LookThroughIntrinsics) {
    ...
    if (CB->getIntrinsicID() == Intrinsic::noalias ||
        CB->getIntrinsicID() == Intrinsic::experimental_ptr_provenance) {
