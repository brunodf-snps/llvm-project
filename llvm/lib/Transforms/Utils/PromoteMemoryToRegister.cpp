...
public:
  bool isChecker() const { return true; }
  ...
  bool wasPHINodeHandled(PHINode* P) {
    return !HandledPhis.insert(P).second;
  }

public:
  SmallPtrSet<PHINode*, 2> HandledPhis;
};

class PromotableTracker {
public:
  bool isChecker() const { return false; }
...
bool isAndOnlyUsedByNoaliasOrProvenanceNoAliasIdentifyPArg(IntrinsicInst *II,
  ...
  if (II->getIntrinsicID() == Intrinsic::provenance_noalias) {
    ...
    return true;
  } else if (II->getIntrinsicID() == Intrinsic::experimental_ptr_provenance) {
    if (!onlyUsedByNoaliasOrProvenanceNoAliasIdentifyPArg(II, pt))
      return false;
    pt.trackRemovable(II);
    return true;
...
bool onlyUsedByNoaliasOrProvenanceNoAliasIdentifyPArg(Value *V, PT &pt) {
  for (Use &U_ : V->uses()) {
    // Remember: either 'return false' or 'continue'. 'return true' is never
    // valid inside this loop.
    ...
    if (IntrinsicInst *II = dyn_cast<IntrinsicInst>(U)) {
      ...
      // Ok if this is the PtrProvenance operand
      if (OpNo != LI->getPtrProvenanceOperandIndex())
        return false;
      pt.trackOperandToZero(LI, OpNo);
    } else if (StoreInst *SI = dyn_cast<StoreInst>(U)) {
      if (OpNo != SI->getPtrProvenanceOperandIndex())
        return false;
      ...
      pt.trackOperandToZero(SI, OpNo);
      if (pt.isChecker())
        if (!onlyUsedByNoaliasOrProvenanceNoAliasIdentifyPArg(SI, pt))
          return false;
    } else if (PHINode *PN = dyn_cast<PHINode>(U)) {
      pt.trackOperandToZero(PN, OpNo);
      if (pt.wasPHINodeHandled(PN))
        continue;
      if (pt.isChecker())
        if (!onlyUsedByNoaliasOrProvenanceNoAliasIdentifyPArg(PN, pt))
          return false;
    } else {
      return false;
    }
...
template <typename PT> bool trackAllocaPromotable(AllocaInst *AI, PT &pt) {
  ...
  for (Use &U_ : AI->uses()) {
    ...
    if (LoadInst *LI = dyn_cast<LoadInst>(U)) {
      ...
      if (pt.check(LI->isVolatile() || LI->getType() != AI->getAllocatedType()))
        return false;
      if (OpNo == LI->getPtrProvenanceOperandIndex()) {
      ...
      if (pt.check(SI->getValueOperand()->getType() != AI->getAllocatedType()))
        return false; // Don't allow a store OF a different type into the AI
      ...
      if (pt.check(!onlyUsedByLifetimeMarkersOrDroppableInsts(ASCI)))
        return false;

      pt.trackRemovableOrDroppableUses(ASCI);
      pt.trackRemovable(ASCI);
    } else {
      return false;
    }
...
static void removeIntrinsicUsers(AllocaInst *AI) {
  ...
  if (NoAliasDecl) {
    ...
    DenseMap<uint64_t, Instruction *> ObjId2NoAliasDecl;
...
static bool rewriteSingleStoreAlloca(
  ...
  for (auto UI = AI->user_begin(), E = AI->user_end();
       UI != E;) { // FIXME: make_early_inc_range ?
