...
void CodeGenFunction::LexicalNoAliasInfo::addNoAliasMD() {
  ...
  llvm::MDNode *NewScopeList = nullptr;
  for (auto &VH : MemoryInsts) {
    if (auto *I = dyn_cast_or_null<llvm::Instruction>(VH)) {
      if (!NewScopeList)
        NewScopeList =
            llvm::MDNode::get(I->getParent()->getContext(), NoAliasScopes);
      I->setMetadata(
